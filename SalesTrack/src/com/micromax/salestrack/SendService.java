 ...
 * Description:
 * ------------
 * This file creates a file macadressss.log at location /mnt/sdcard/Android/data/com.micromax.tabletinfo ,writes a serial no of a device .
 *
 * Author:
 * -------
 @author dh.anant 
...
public class SendService extends Service {
	...
	{  
		...
		if(Setting.IS_WIFIONLY_TABLET){
			sendCreateFile();
		}
		System.out.println("checK:" +getSharedPreferences("timertask", 0).getString("startservice", ""));
		System.out.println("checK:" +getSharedPreferences("locValues", 0).getString("Serverloc", ""));

		//message sent wifi
		//message sent sim
		if( !getSharedPreferences("locValues", 0).getString("Serverloc","").equals("message sent sim"))
		{
			System.out.println("checK:starting the TimerTask Service");
			Intent checkIp = new Intent(getBaseContext(),TimerTaskService.class);    
			checkIp.setAction("Start");
			startService(checkIp);	
		}else{

			System.out.println("checK:else" +getSharedPreferences("timertask", 0).getString("startservice", ""));
	 ...
	 * 
	 * 
	 * DESCRIPTION This method creates a file and write mac adress into it
	 * 
	 * @param boolean
	 * @return void
	 * @author dh.anant
...
*/		// String sFile="/macadressss.log";
		String sFile = "/Android/data/com.micromax.Tabletinfo/macadressss.txt";
		...
		TelephonyManager tMgr = (TelephonyManager) getApplicationContext()
				.getSystemService(SendService.TELEPHONY_SERVICE);
		String networkOperator = tMgr.getNetworkOperator();

		System.out.println("the value of network operator is "
				+ networkOperator);
		deviceIdImei = tMgr.getDeviceId();

		// if(deviceIdImei.contains(Setting.TAC_ID_TABLET))
		// {
		...
		if (!getSharedPreferences("timertask", 0).getString("startservice", "")
				.equals("timerservicestarted")) {
			System.out.println("starting the TimerTask Service");
			Intent checkIp = new Intent(getBaseContext(),
					TimerTaskService.class);
			checkIp.putExtra("PRID", serialNO);
			checkIp.setAction("Start");
			startService(checkIp);
		}
	...
	public void writeInFile(String serialNumber) {
		...
		try {
			...
			if (serialNumFile.exists()) {
				...
				//Toast.makeText(mContext, "File not created ", Toast.LENGTH_LONG).show();
				System.out.println("File not created ");
			}
		} catch (Exception e) {
	...
	private File createFile(String exStorageDirectory, File myFile) {
		if (!myFile.exists()) {
			try {
				...
				File androidDataPath = new File(exStorageDirectory
						+ sAndroidDataPath);
				...
				File tabletInfoPath = new File(exStorageDirectory
						+ sTabletInfoPath);
				...
				System.out
				.println("created /Android/data/com.micromax.Tabletinfo");
				if (bFolderCreatesuccess) {
				/*	myFile = new File(
							Environment.getExternalStorageDirectory()
							+ File.separator
							+ "Android/data/com.micromax.Tabletinfo/macadressss.log");*/
					myFile = new File(
							Environment.getExternalStorageDirectory()
							+ File.separator
							+ "Android/data/com.micromax.Tabletinfo/macadressss.txt");
					// myFile= new
					// File(Environment.getExternalStorageDirectory() +
					// File.separator+"macadressss.log");
	 ...
	 * 
	 * 
	 * DESCRIPTION This method generates a integer val.
	 * 
	 * @param String
	 * @return String
	 * @author dh.anant
	 ...
	 * 
	 * 
	 * DESCRIPTION This method generates a checksum for the 15 digit serial
	 * number.
	 * 
	 * @param String
	 * @return String
	...
	public static String generate_checkdigit(String Inst) {
		...
		for (k = Inst1.length(); (k > 1); k = k - 2) {
			LH = LH
					+ Integer
					.toHexString(hextoInt(Inst1.substring((k - 1), k)) * 2);
			LH.toUpperCase();
		}
		RH = "";
	 ...
	 * 
	 * 
	 * DESCRIPTION This method encrypt the string.
	 * 
	 ...
	 * 
	 * 
	 * DESCRIPTION This method to process the key specified for encryption and
	 * decryption.
	 * 
	 * @param String
	 * @return byte[]
	 ...
	 * 
	 * 
	 * DESCRIPTION This method encrypts a string with a key.
	 * 
	 * @param String
	 * @return String
	 * @author dh.anant
	 ...
	 * 
	 * 
	 * DESCRIPTION This method concatenates TAC_ID for every macadress.
	 * 
	 * @param String
	 * @return String
	 * @author dh.anant
	 ...
	 * 
	 * 
	 * DESCRIPTION This method writes a encrypted string into a file .
	 * 
	 * @param String
	 * @return String
	 * @author dh.anant
